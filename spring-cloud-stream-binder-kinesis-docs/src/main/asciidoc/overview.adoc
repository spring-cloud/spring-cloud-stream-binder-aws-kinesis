...
+
No default - if not set, default value inside spring-integration-aws will be used (5000).
...
checkpointMode::
  ...
  If `manual`, checkpoints occur on demand via the `Checkpointer` callback. If `periodic`, checkpoints occurs at specified time interval (from `interval` property in checkpoint configuration)
+
Default: `batch`
workerId::
