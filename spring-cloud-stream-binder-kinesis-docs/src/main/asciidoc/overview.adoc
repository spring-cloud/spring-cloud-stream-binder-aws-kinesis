...
With Kinesis Client Library we can subscribe only to the provided stream and shards distribution is done by that client.
...
Can be set to `0` or negative value to disable heartbeat feature altogether.
The `KinesisMessageDrivenChannelAdapter` does it own locks renewal eitherway.
+
Default: `5`
refreshPeriod::
