...
Default: `1`
kplKclEnabled::
    Enable the usage of https://docs.aws.amazon.com/streams/latest/dev/developing-consumers-with-kcl-v2.html[KCL] / https://docs.aws.amazon.com/streams/latest/dev/developing-producers-with-kpl.html[KPL] for all message consumption and production
+
Default: `false`
...
The functionality is based on the https://github.com/awslabs/dynamodb-streams-kinesis-adapter[DynamoDB Streams Kinesis Adapter] and every Kinesis consumer endpoint selects an appropriate Kinesis Client according the `spring.cloud.stream.kinesis.bindings.<channelName>.consumer.dynamoDbStreams` boolean flag.
In case of DynamoDB Streams the `KinesisStreamProvisioner` throws a `ProvisioningException` if there is no requested stream in the DynamoDB: the DynamoDB Streams Kinesis Adapter doesn't support stream entity manipulation operations.
