...
shardId::
The `String` value indicating Kinesis consumer channel adapter should use specific shardId for consuming messages.
For example: `shardId:shardId-000000000001`.
+
No default - means no specific shardId selected.

NOTE: KCL doesn't expose any option to deal with a specific shard. When `shardId` property is used, it won't work with KCL. Also, in case
`instanceCount`>1, application will throw validation exception. So, `instanceCount` and `shardId` are considered as mutually exclusive.
